...
```sh
hamilton-vault build-warehouse examples/replacement/project.json --warehouse examples/warehouses/databricks-vault.json --output build/databricks
# Executes and writes remotely; not run in the offline tests:
hamilton-vault run-warehouse build/databricks
```

This is the v2 metadata path (stage, hub, link, satellite), using the existing Hamilton dependency graph. `source_tables` maps metadata source names to landing relations. `target_schema` stores vault tables; `staging_schema` stores helper views. Update `load_dts` for each batch. The original v1/local CSV path remains DuckDB-specific.

## Validation record

The local regression suite passed **77 tests** on 2026-09-18. The checked-in [JUnit report](reports/warehouse-regression.xml) includes DuckDB regressions, offline warehouse compilation and driver stubs. No Databricks, Snowflake or ClickHouse service was contacted. SQL parsing establishes compilation coverage, not live backend certification.
